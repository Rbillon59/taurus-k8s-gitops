...
usage()
{
  ...
  logit "INFO" "example : ./deploy_taurus.sh -n taurus -p my-scenario"
  exit 1
}
...
logit "INFO" "Creating taurus slave nodes"
...
logit "INFO" "Creating taurus deployments"
kubectl create -n "${namespace}" -f "deploy_taurus_slaves.yaml"
kubectl -n "${namespace}" scale statefulsets taurus-slaves --replicas=${nb_injectors}

logit "INFO" "Waiting for slaves to be up and ready"
logit "INFO" "$(kubectl get -n ${namespace} all)"

kubectl -n "${namespace}" rollout status StatefulSet/taurus-slaves

logit "INFO" "All slaves are ready, starting the taurus controller"

kubectl create -n "${namespace}" -f "deploy_taurus_master.yaml"
